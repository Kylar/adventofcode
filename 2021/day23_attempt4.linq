...
// Same as (or similar to) attempt3 but instead of the minheap I found on a random gist, I'm using the same fast priority queue nuget package as attempt2.
...
void gen_moves(byte[] state /* length usize = 64 */, List<(int start, int end, int dist)> moves)
{
	...
	void try_gen_move(List<(int, int, int)> moves, int hallway, int room, bool to_room)
	{
		...
		//room_first_occupied.Dump("room_first_occupied");
		...
		else
		{
			...
			if (nullable_occupied_idx != null)
			{
				...
				//path_len.Dump("path len");
				//target_room.Dump("target room");
				...
				//new { hallway, room, to_room, direct_route, path_len, target_room }.Dump();
...
class PriorityQueueNode<TValue> : StablePriorityQueueNode
{
	public TValue Value;
}
...
int? astar_fuel_cost(byte[] initial_state)
{
	...
	var stateToNode = new Dictionary<HeapEntry, PriorityQueueNode<HeapEntry>>();
	PriorityQueueNode<HeapEntry> GetNode(HeapEntry node)
	{
		if (stateToNode.TryGetValue(node, out var result))
		{
			return result;
		}
		var r = new PriorityQueueNode<HeapEntry>();
		r.Value = node;
		stateToNode.Add(node, r);
		return r;
	}
	var to_visit = new StablePriorityQueue<PriorityQueueNode<HeapEntry>>(40000);
	to_visit.Enqueue(GetNode(new HeapEntry(0, initial_state)), 0);
	...
	while (to_visit.Count > 0)
	{
		...
		var (cost, _state) = to_visit.Dequeue().Value;
		//(_hcost, cost, _state).Dump("node");
		...
		//moves.Dump("moves before");
		gen_moves(_state, moves);
		//moves.Dump("moves after");
		foreach (var tuple in moves)
		{
			...
			if (new_cost < (min_cost.TryGetValue(new_state, out var min_cost_value2) ? min_cost_value2 : long.MaxValue))
			{
				...
				to_visit.Enqueue(GetNode(new HeapEntry(new_cost, new_state)), heuristic_cost);
			}
		}
		moves.Clear();
	}
...
void Main()
{
	...
	/* debug code */
	//var part1state = parse_state(input, 11 + 2 * 4).Dump();
	//// Move Ds to right hallway
	//part1state[10] = 4;
	//part1state[11] = 0;
	//part1state[9] = 4;
	//part1state[12] = 0;
	...
	//input.Dump();
	//var moves = new List<(int,int,int)>();
	//gen_moves(part1state, moves);
	//moves.Dump("moves from initial state");
	////heuristic_fuel_cost(part1state).Dump();
	//return;
	/* end debug code */
	...
	Measure(() => astar_fuel_cost(parse_state(string.Join("", part2_input), 11 + 4 * 4))).Dump("measure part2");
...
struct HeapEntry
{
	...
	public HeapEntry(int cost, byte[] state)
	{
		this.cost = cost;
		this.state = state;
	}

	public void Deconstruct(out int cost, out byte[] state)
	{
		cost = this.cost;
		state = this.state;
	}
